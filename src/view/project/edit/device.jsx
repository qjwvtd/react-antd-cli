...
import { Table, Row, Col, Input, Select, Button, Icon, Pagination, message } from 'antd';
...
//设备库存列表
const DeviceStockTable = observer(({ id }) => {
    ...
    function setDeviceSellList(option, e) {
        ...
        function getSellItem(sid) {
            for (let i = 0; i < sellArr.length; i++) {
                if (sellArr[i].typeId === sid) {
                    return { index: i };
                }
            }
            return false;
...
const DeviceAllotedTable = observer(({ id }) => {
    editStore.initAllotedDeviceList(id);
    const [deviceId, setDeviceId] = useState(null);
    const [deviceTypeId, setDeviceTypeId] = useState(null);
    //查询
    function query() {
        editStore.setDeviceSearchContent(deviceId, deviceTypeId);
        editStore.initAllotedDeviceList(id);
    }
    //回收
...
const DeviceInfo = observer(({ id }) => {
    return <Fragment>
        <div className="ui-matchDevices-container">
            <DeviceStockTable id={id} />
            <DeviceAllotedTable id={id} />
